...
describe("llms.txt + OpenAPI SEO surfaces", () => {
  it("ships llms.txt and docs/openapi.yaml at known paths", () => {
    ...
    assert.match(text, /GET \/openapi\.yaml/);
  ...
  it("GET /llms.txt and /.well-known/llms.txt return the same plain text", async () => {
    ...
    assert.match(a.text, /npx x402-micro-tollgate@0\.3\.3 --seller/);
    assert.match(a.text, /GET \/openapi\.yaml/);
    assert.equal(a.text, b.text);
  });

  it("GET /openapi.yaml and /docs/openapi.yaml return the same YAML", async () => {
    const config = loadConfig({
      X402_PAY_TO: "0x1234567890123456789012345678901234567890",
    });
    const { app } = await createApp({
    ...
    const a = await request(app).get("/openapi.yaml");
    const b = await request(app).get("/docs/openapi.yaml");
    assert.equal(a.status, 200);
    assert.equal(b.status, 200);
    const ct = String(a.headers["content-type"]);
    assert.ok(
      /yaml|text\//i.test(ct),
      `expected yaml or text content-type, got ${ct}`,
    );
    assert.match(a.text, /openapi:\s*3\.1\.0/);
    assert.equal(a.text, b.text);
  });
});
